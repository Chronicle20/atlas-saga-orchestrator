...
type Processor interface {
	...
	RequestCreateCharacter(transactionId uuid.UUID, accountId uint32, name string, worldId byte, channelId byte, jobId uint32, face uint32, hair uint32, hairColor uint32, skin uint32, top uint32, bottom uint32, shoes uint32, weapon uint32) error
}
...
func (p *ProcessorImpl) RequestCreateCharacter(transactionId uuid.UUID, accountId uint32, name string, worldId byte, channelId byte, jobId uint32, face uint32, hair uint32, hairColor uint32, skin uint32, top uint32, bottom uint32, shoes uint32, weapon uint32) error {
	return message.Emit(p.p)(func(mb *message.Buffer) error {
		return mb.Put(character2.EnvCommandTopic, RequestCreateCharacterProvider(transactionId, accountId, name, worldId, channelId, jobId, face, hair, hairColor, skin, top, bottom, shoes, weapon))
	})
}
